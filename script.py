...
# Save the results as CSV files in the year folder
    ...
    if min(merged_similarities.shape[0], merged_similarities.shape[1]) > 1:
        ...
        #print("Sum of explained variance with 2 PCs:", round(sum(pca.explained_variance_ratio_), 2))
    ...
    ## Get cosine distances based on the whole similarities dataframe (no PCA applied)
    ...
    # Display velocities for each month pair
    #for month_pair, velocity_df in velocities_cosine_full.items():
        #print(f"Velocities for {month_pair}:")
        #print(velocity_df.head(10))  # Display the first few rows
        #print("-------------------------------")
    ...
    # for column in velocities_df_full.columns:
    #     nan_count = velocities_df_full[column].isna().sum()
    ...
    # Print results
    #for month in month_names:
        #print(f"{month}: Average std = {avg_std_per_month[month]:.4f}, Average mean = {avg_mean_per_month[month]:.4f}")
    ...
    #print("---------------------")
    #for month, mod in modularity_per_month.items():
        #print(f"{month}: {mod:.4f}")
    # Calculate the average modularity for each consecutive month pair
    ...
    # Print results
    #for pair, avg in zip(consecutive_month_pairs, consecutive_modularity_averages):
        #print(f"{pair}: {avg:.4f}")
    ...
    # Calculate the average similarity mean and std for each consecutive month pair
    consecutive_similarity_mean_averages = []
    consecutive_similarity_std_averages = []
    consecutive_month_pairs = []

    months = list(avg_mean_per_month.keys())
    mean_similarities = list(avg_mean_per_month.values())
    std_similarities = list(avg_std_per_month.values())

    for i in range(len(months) - 1):
        mean_avg = (mean_similarities[i] + mean_similarities[i + 1]) / 2
        std_avg = (std_similarities[i] + std_similarities[i + 1]) / 2
        consecutive_similarity_mean_averages.append(mean_avg)
        consecutive_similarity_std_averages.append(std_avg)
        consecutive_month_pairs.append(f"{months[i]}-{months[i+1]}")

    # Prepare velocity means and stds for the same x-axis
    means_arr = np.array([velocities_df_full[col].mean(skipna=True) for col in column_order])
    stds_arr = np.array([velocities_df_full[col].std(skipna=True) for col in column_order])

    # Combined plot with twin axes
    fig, ax1 = plt.subplots(figsize=(12, 6))

    # Plot average modularity on primary y-axis
    ax1.plot(consecutive_month_pairs, consecutive_modularity_averages, marker='o', linestyle='-', color='purple', label='Avg. Modularity')

    # Plot similarity mean ± std fill on the primary axis
    mean_sim = np.array(consecutive_similarity_mean_averages)
    std_sim = np.array(consecutive_similarity_std_averages)


    ax1.set_xlabel('Month Pair')
    ax1.set_ylabel('Average Modularity', color='purple')
    ax1.tick_params(axis='y', labelcolor='purple')
    ax1.set_xticks(range(len(consecutive_month_pairs)))
    ax1.set_xticklabels(consecutive_month_pairs, rotation=45)

    # Create secondary y-axis for velocity stats
    ax2 = ax1.twinx()
    ax2.plot(column_order, means_arr, marker='o', linestyle='--', color='g', label='Mean Velocity')
    ax2.fill_between(column_order, means_arr - stds_arr, means_arr + stds_arr, color='green', alpha=0.15, label='Velocity Mean ± Std. Dev.')
    ax2.plot(consecutive_month_pairs, mean_sim, marker='s', linestyle='-', color='gray', label='Avg. Mean Similarity (right axis)')
    ax2.fill_between(consecutive_month_pairs, mean_sim - std_sim, mean_sim + std_sim, color='gray', alpha=0.2, label='Similarity Mean ± Std. Dev. (right axis)')
    ...
    plt.title(f'{year}, Average Louvain Modularity, Similarity, and Velocity Statistics per Month Pair')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(year_folder, f"combined_{year}.png"))
    #plt.show()
